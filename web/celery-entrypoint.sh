...
if [ ! -f /usr/src/wordlist/combined_subdomains.txt ];
then
  echo "Downloading Combined_Subdomains.txt from Seclists"
  wget https://raw.githubusercontent.com/danielmiessler/SecLists/master/Discovery/DNS/combined_subdomains.txt -O /usr/src/wordlist/combined_subdomains.txt
fi

# check if resolver and download if not
if [ ! -f /usr/src/wordlist/dns-resolvers.txt ];
then
  echo "Downloading DNS-Resolver From Seclists"
  wget https://raw.githubusercontent.com/danielmiessler/SecLists/master/Miscellaneous/dns-resolvers.txt -O /usr/src/wordlist/dns-resolvers.txt
fi
